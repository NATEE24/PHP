...
// ลบสินค้า (พร้อมลบไฟล์รูปด้วย)
if (isset($_GET['delete'])) {
    $product_id = (int) $_GET['delete']; // แปลงค่าเป็นตัวเลข (int)
    // 1) ดึงชื่อไฟล์รูปจากฐานข้อมูลก่อน
    $stmt = $conn->prepare("SELECT image FROM products WHERE product_id = ?");
    $stmt->execute([$product_id]);
    $imageName = $stmt->fetchColumn(); // จะได้ null ถ้าไม่มีรูป
    // 2) ลบข้อมูลในฐานข้อมูล โดยใช้ Transaction
    try {
        ...
        $conn->rollBack();
        // ใส่ flash message หรือบันทึก log ได้ตามต้องการ
        header("Location: products.php");
        exit;
    }
    // 3) ลบไฟล์รูป หลังจากลบข้อมูลใน DB สำเร็จแล้ว
    if ($imageName) {
        $baseDir = realpath(__DIR__ . '/../product_images'); // โฟลเดอร์เก็บรูปสินค้า
        $filePath = realpath($baseDir . '/' . $imageName);

        // ตรวจสอบป้องกัน path traversal: ต้องอยู่ในโฟลเดอร์ $baseDir จริง ๆ
        if ($filePath && strpos($filePath, $baseDir) === 0 && is_file($filePath)) {
            @unlink($filePath); // ใช้ @ เพื่อปิด warning ถ้าลบไม่สำเร็จ
        }
    }
                            ...
                            <?php foreach ($products as $p): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="edit_products.php?product_id=<?= $p['product_id'] ?>"
