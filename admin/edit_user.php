...
if (!$user) {
    // ใช้สไตล์เดียวกับหน้าอื่น ๆ (โทนชมพูอ่อน)
    echo '<!doctype html><html lang="th"><head>
            ...
            <style>
              body{
                background-color:#ffe4ec; /* ชมพูอ่อน */
                min-height:100vh;margin:0;display:flex;align-items:center;justify-content:center;padding:24px 12px;
              }
              .page-box{
                background:#fff;width:100%;max-width:700px;border-radius:12px;
                box-shadow:0 6px 20px rgba(0,0,0,.20);padding:24px;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ตรวจซ้ำ (username/email ชนกับคนอื่น)
    if (!$error) {
...
<head>
    ...
    <style>
        body{
            background-color:#ffe4ec; /* ชมพูอ่อนพาสเทล */
            min-height:100vh;margin:0;display:flex;align-items:flex-start;justify-content:center;padding:24px 12px;
        }
        .page-box{
        ...
        .card-modern{
            border:1px solid rgba(255,105,180,.20);
            border-radius:12px;
            box-shadow:0 4px 14px rgba(255,105,180,.15);
            background:#fff0f5; /* lavender blush */
        }
        .card-modern:hover{ background:#ffffff; }

        .dash-title{
            font-size:1.8rem;font-weight:700;margin-bottom:.25rem;text-align:center;
            color:#d63384; /* หัวข้อสีชมพูเข้ม */
        }
        .dash-sub{
            text-align:center;color:#6c757d;margin-bottom:1.25rem;
        }
        .top-bar{
            display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:1rem;
        }

        /* ตารางหัวตารางชมพูอ่อน */
        .table thead th{
            background:#ffe6f2;
        ...
        /* ปุ่มเตือน/ลบให้ไปกับธีม */
        .btn-warning{ color:#6b2c36; } /* อ่านง่ายบนพื้นชมพู */
    </style>
</head>
<body>
